import{a as c}from"./api.DUBSwrnZ.js";import"./hoisted.DbckvgP9.js";const d=document.getElementById("entrepreneurship-list");async function m(){if(d)try{const s=await c.getEntrepreneurships(),i=s.entrepreneurships||s;if(!i||i.length===0){d.innerHTML=`
                    <div class="col-span-full text-center py-20 bg-slate-50 rounded-[2.5rem] border-2 border-dashed border-slate-200 flex flex-col items-center justify-center my-8">
                        ...
                        <h2 class="text-xl font-bold text-slate-800 mb-2">Comienza tu viaje</h2>
                        <p class="text-slate-500 mb-8 max-w-xs mx-auto text-sm font-medium">Registra tu primer emprendimiento para empezar a proyectar tus planes económicos con KEIKAKU.</p>
                        <a href="/entrepreneurship/create" class="btn btn-primary btn-sm px-10 shadow-lg shadow-indigo-100">Crear mi primer proyecto</a>
                    </div>
                `;return}d.innerHTML=i.map(e=>{const t=e.logoUrl||e.imageUrl||e.logo||e.image,l="http://localhost:3000";let o=null;return t&&typeof t=="string"&&t.trim()!==""&&(o=t.startsWith("http")?t:`${l}${t.startsWith("/")?"":"/"}${t}`),`
                <div class="group bg-white rounded-[2.5rem] overflow-hidden border border-slate-100 shadow-xl shadow-slate-200/50 hover:shadow-2xl hover:shadow-indigo-100 hover:-translate-y-2 transition-all duration-500 flex flex-col h-full relative">
                    ...
                    <div class="p-8 flex-1 flex flex-col">
                        ...
                        <div class="mt-auto flex items-center gap-3 pt-6 border-t border-slate-50">
                            <a href="/plan/create?entId=${e.id}" class="btn btn-primary btn-sm flex-1 shadow-lg shadow-indigo-100 text-sm py-3 active:scale-95 transition-all">
            ...
            `}).join(""),document.querySelectorAll(".btn-delete-ent").forEach(e=>{e.addEventListener("click",t=>{const o=t.currentTarget.dataset.id;b("delete-entrepreneurship",o)})}),document.querySelectorAll(".btn-edit-ent").forEach(e=>{e.addEventListener("click",async t=>{const o=t.currentTarget.dataset.id;try{const r=await c.getEntrepreneurship(parseInt(o));window.dispatchEvent(new CustomEvent("open-entrepreneurship-wizard",{detail:{mode:"edit",data:r}}))}catch{alert("Error al cargar datos del emprendimiento")}})})}catch(s){console.error(s),d.innerHTML='<p class="text-red-500">Error al cargar listado.</p>'}}const u=document.getElementById("security_confirm_modal"),g=document.getElementById("security-confirm-form"),p=document.getElementById("security-password"),h=document.getElementById("security-error"),a=document.getElementById("security-submit-btn");function b(s,i){document.getElementById("security-action-type").value=s,document.getElementById("security-target-id").value=i,p.value="",h?.classList.add("hidden"),u?.showModal()}g?.addEventListener("submit",async s=>{s.preventDefault();const i=document.getElementById("security-action-type").value,e=document.getElementById("security-target-id").value,t=p.value;a&&(a.innerHTML='<span class="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></span>',a.disabled=!0);try{await c.verifyPassword(t),i==="delete-entrepreneurship"&&(await c.deleteEntrepreneurship(parseInt(e)),m()),u?.close()}catch(l){console.error(l),h?.classList.remove("hidden")}finally{a&&(a.innerHTML="<span>Confirmar y Eliminar</span>",a.disabled=!1)}});document.getElementById("security-cancel-btn")?.addEventListener("click",()=>u?.close());document.getElementById("btn-close-security-modal")?.addEventListener("click",()=>u?.close());const v=document.getElementById("btn-new-entrepreneurship");v?.addEventListener("click",()=>{window.dispatchEvent(new CustomEvent("open-entrepreneurship-wizard",{detail:{mode:"create"}}))});window.addEventListener("entrepreneurship-success",()=>{m()});m();
